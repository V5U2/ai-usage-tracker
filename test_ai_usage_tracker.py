...
class ServerHttpTests(unittest.TestCase):
    ...
    def test_admin_ui_creates_renames_revokes_and_hashes_token(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Issue Collector Token", body)
            self.assertIn("Existing Collector Tokens", body)
            self.assertIn("Issue Admin API Key", body)
            self.assertIn("Existing Admin API Keys", body)

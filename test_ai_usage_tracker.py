...
class ServerHttpTests(unittest.TestCase):
    ...
    def test_admin_ui_creates_renames_revokes_and_hashes_token(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Collector Tokens", body)
    ...
    def test_admin_api_keys_can_be_managed_and_authenticate_report_api(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            body = app.ServerReceiver.render_admin(object(), con, token=token, token_label="New admin API key")
            self.assertIn("Admin API Keys", body)
            self.assertIn("New admin API key, shown once", body)
    ...
    def test_oidc_exchange_rejects_private_discovered_endpoints_before_request(self):
        ...
        with patch.object(core, "oidc_discovery", return_value=discovery), patch.object(core, "oidc_urlopen") as urlopen:
            with self.assertRaises(ValueError):
                app.oidc_exchange_code(config, "code")

        urlopen.assert_not_called()

    def test_oidc_fetch_rejects_redirects(self):
        handler = app.OidcNoRedirectHandler()

        with self.assertRaisesRegex(ValueError, "redirects"):
            handler.redirect_request(None, None, 302, "Found", {}, "https://10.0.0.5/token")
